...
START_TEST(acos_09) {
  for (double i = 0.9; i > -1; i -= 0.4) {
    ck_assert(!precision_check(s21_acos(i), acos(i)));
...
START_TEST(acos_1) {
  ck_assert(!precision_check(s21_acos(1), acos(1)));
  ck_assert(!precision_check(s21_acos(-1), acos(-1)));
...
START_TEST(acos_100) {
  for (double i = 100; i >= -100; i -= 10) {
    if (i) ck_assert(!precision_check(s21_acos(i), acos(i)));
...
START_TEST(acos_NAN) {
  // NAN
  ck_assert(!precision_check(s21_acos(S21_NAN), acos(S21_NAN)));
...
START_TEST(acos_INFINITY) {
  // NAN
  ck_assert(!precision_check(s21_acos(S21_INF), acos(S21_INF)));
