...
START_TEST(test_s21_math_pow_usual) {
  ...
  ck_assert_ldouble_eq_tol(s21_pow(1.6, 8.7), pow(1.6, 8.7), 1e-6);
  for (double i = 7.2; i < 10.0; i += 0.1) {
    for (double j = 0.0; j < 10.0; j += 0.1) {
      // printf("\n%f %f\n\n", i, j);
      // printf("\n%.7Lf %.7f\n\n", s21_pow(i, j), pow(i, j));
      ck_assert_ldouble_eq_tol(s21_pow(i, j), pow(i, j), 1e-6);
    }
  }
}
END_TEST
...
START_TEST(test_s21_math_pow_cases_should_return_nan) {
  for (unsigned long i = 0; i < sizeArr(cases_pow_return_nan); i++) {
...
START_TEST(test_s21_math_pow_cases_should_return_inf) {
  for (unsigned long i = 0; i < sizeArr(cases_pow_return_inf); i++) {
...
START_TEST(test_s21_math_pow_cases_should_return_one) {
  for (unsigned long i = 0; i < sizeArr(cases_pow_return_one); i++) {
