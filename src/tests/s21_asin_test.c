...
START_TEST(asin_09) {
  for (double i = 0.9; i > -1; i -= 0.4) {
    ck_assert(!precision_check(s21_asin(i), asin(i)));
...
START_TEST(asin_1) {
  ck_assert(!precision_check(s21_asin(1), asin(1)));
  ck_assert(!precision_check(s21_asin(-1), asin(-1)));
...
START_TEST(asin_100) {
  for (double i = 100; i >= -100; i -= 10) {
    if (i) ck_assert(!precision_check(s21_asin(i), asin(i)));
...
START_TEST(asin_NAN) {
  // NAN
  ck_assert(!precision_check(s21_asin(S21_NAN), asin(S21_NAN)));
...
START_TEST(asin_S21_INF) {
  // NAN
  ck_assert(!precision_check(s21_asin(S21_INF), asin(S21_INF)));
