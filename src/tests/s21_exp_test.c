...
START_TEST(exp_fn) {
  ck_assert(!precision_check(s21_exp(5), exp(5)));
  ck_assert(!precision_check(s21_exp(-2), exp(-2)));
  ck_assert(!precision_check(s21_exp(1234567), exp(1234567))); // INF
  ck_assert(!precision_check(s21_exp(-1234567), exp(-1234567)));
  for (double i = -10; i <= 10; i++) {
    // printf("\n%.6Lf\n%.6f\n\n", s21_exp(i), exp(i)); //валится на s21_exp(10), раскомментировать для дебага
    ck_assert(!precision_check(s21_exp(i), exp(i)));
  }
  ck_assert(!precision_check(s21_exp(0.0), exp(0.0)));
  ck_assert(!precision_check(s21_exp(1.0), exp(1.0)));
  ck_assert(!precision_check(s21_exp(-1.0), exp(-1.0)));
  ck_assert(!precision_check(s21_exp(10.0), exp(10.0)));
  ck_assert(!precision_check(s21_exp(-10.0), exp(-10.0)));
  ck_assert(!precision_check(s21_exp(-100.0), exp(-100.0)));
  ck_assert(!precision_check(s21_exp(-1000.0), exp(-1000.0)));
  ck_assert(!precision_check(s21_exp(0.1), exp(0.1)));
  ck_assert(!precision_check(s21_exp(-0.1), exp(-0.1)));
...
START_TEST(s21_exp_inf) {
  // INF
  ck_assert(!precision_check(s21_exp(S21_INF), exp(S21_INF)));
...
START_TEST(s21_exp_nan) {
  // NAN
  ck_assert(!precision_check(s21_exp(S21_NAN), exp(S21_NAN)));
...
START_TEST(s21_exp_max_double) {
  // INF
  ck_assert(!precision_check(s21_exp(S21_double_MAX), exp(S21_double_MAX)));
...
START_TEST(s21_exp_min_double) {
  // 1.0
  ck_assert(!precision_check(s21_exp(S21_double_MIN), exp(S21_double_MIN)));
