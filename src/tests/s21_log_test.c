...
START_TEST(log_fn) {
  ck_assert(!precision_check(s21_log(1.1), log(1.1)));
  // ck_assert_ldouble_eq_tol(s21_log(1.1), log(1.1), S21_EPS);
  for (double i = 1.; i < 10; i += 0.1) {
    // printf("\n%f\n\n", i);
    ck_assert(!precision_check(s21_log(i), log(i)));
...
START_TEST(log_fn_comparison) {
  ck_assert(!precision_check(s21_log(3.14), log(3.14)));
  // double x = 3.14;
...
START_TEST(log_fn_extra) {
  ck_assert(!precision_check(s21_log(2.0), log(2.0)));
...
START_TEST(s21_log_negative) {
  // -NAN
  ck_assert(!precision_check(s21_log(-2), log(-2)));
  // ld_bits check = {log(-2)};
...
START_TEST(s21_log_zero) {
  // -INF
  ck_assert(!precision_check(s21_log(0.), log(0.)));
...
START_TEST(s21_log_inf) {
  // INF
  ck_assert(!precision_check(s21_log(S21_INF), log(S21_INF)));
...
START_TEST(s21_log_nan) {
  // NAN
  ck_assert(!precision_check(s21_log(S21_NAN), log(S21_NAN)));
...
START_TEST(s21_log_max_double) {
  // 709.782713
  ck_assert(!precision_check(s21_log(S21_double_MAX), log(S21_double_MAX)));
...
START_TEST(s21_log_min_double) {
  // -709.782713
  ck_assert(!precision_check(s21_log(S21_double_MIN), log(S21_double_MIN)));
